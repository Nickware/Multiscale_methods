...
- **Monte Carlo de Importancia**: Reescribe integrales como expectativas bajo la distribución tilted $q(x)$ que enfatiza regiones de alta contribución, reduciendo la varianza: $\mathbb{E}_q [f(x) w(x)]$ con $w(x) = p(x)/q(x)$.[wikipedia](https://en.wikipedia.org/wiki/Monte_Carlo_method)
- **Markov Chain Monte Carlo (MCMC)**: Explora distribuciones $p(x)$ vía cadenas de Markov ergódicas, con propuestas de Metropolis-Hastings —aceptación— $(\min(1,p(x')/p(x)))$ o Gibbs sampling para componentes condicionales; clave en inferencia bayesiana y muestreo de Boltzmann en física estadística.[wikipedia](https://en.wikipedia.org/wiki/Monte_Carlo_method)
...
5. **Evaluar convergencia y refinar**: Monitorea el error estadístico $(O(1/\sqrt{N}))$; si es insuficiente, incrementa $N$ o aplica técnicas de reducción de varianza; visualiza distribuciones (histogramas) para validar.[aws.amazon](https://aws.amazon.com/es/what-is/monte-carlo-simulation/)
...
- Contar la proporción $k/N$ dentro del círculo ($x^2 + y^2 \leq 1$).
